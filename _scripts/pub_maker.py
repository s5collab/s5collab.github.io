...
def get_title_str(pub):
    ...
    things_to_fix = [[r"$\sim$", "~"], [r"$R$", "*R*"],
                     [r"[$\alpha/\rm Fe]$", "[α/Fe]"], [r"$\alpha$", "α"],
                     [r"∼", "~"], [r"$< -0.75$", "< −0.75"],
                     [r"$\textit{TESS}$", "*TESS*"], [r"$Gaia$", "*Gaia*"],
                     [r"${S}^5$", "S⁵"], [r"$S^5$", "S⁵"], ["(S5)", "(S⁵)"],
                     ["S<SUP>5</SUP>", "S⁵"], ["S <SUP>5</SUP>", "S⁵"],
                     ["*", r"\*"]]
...
def get_data_frame(library_id):
    ...
    r = requests.post(
        ...
        params={
            "q": "*:*",
            "fl":
            "bibcode,title,year,bibstem,author_count,volume,pub,page,issue,"
            "identifier,author,doi,date,doctype",
            "rows": 2000
        },
        headers={
...
def create_webpage(library_id, md_pub_file, title, subtitle):
    ...
    with open(md_pub_file, 'w') as pub_md:
        ...
        article_list = []
        eprint_list = []
        for year, year_df in pub_df.sort_values(
                'year', ascending=False).groupby("year", sort=False):
            ...
            pub_md.write(f"#### {year}\n")
            pub_md.write("\n")
            for *_, pub in year_df.sort_values(['date', 'bibcode'],
                                               ascending=[False,
                                                          False]).iterrows():
                markdown_str = "* "
                title_str = (
                    f"[**{get_title_str(pub)}**]"
                    f"(https://ui.adsabs.harvard.edu/abs/{quote(pub['bibcode'])})"
                )
                ...
                markdown_str = (
                    f"* {title_str}<br/>{author_str} {year_str}"
                    f" {publication_str} {link_str(get_doi_str(pub), get_arxiv_str(pub))}\n"
                )
